...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
        return Math.sqrt( Math.pow(vx - ux, 2) + Math.pow(vy-uy, 2)); //Pythagorean theorem
  }
  ...
  public void computeAllEuclideanDistances() {
    for (Vertex v: getVertices())
      for (Edge e: v.adjacentEdges) //access all the edges by interating through the vertices' adjacentEdges Lists
  ...
  public void doDijkstra(String s) {
    for (Vertex v: getVertices()){
      v.distance = Double.POSITIVE_INFINITY; // sets all of the costs to positive infinity
      v.known = false; // sets all of the visited booleans to false
      v.prev = null; // sets all of the previous pointers to null
    }
    getVertex(s).distance = 0; //the distance from the vertex to itself is 0
    getVertex(s).known = true; //and the vertex has visited itself
    LinkedList<Vertex> cities = new LinkedList<Vertex>(); //uses a LinkedList instead of a priority queue
    cities.add(getVertex(s)); //insert s into the LinkedList
    Vertex next;
    // while loop adapted from lecture 16 pseudo code
    while( !cities.isEmpty( ) ) {
      next = cities.poll();
      next.known = true; // sets boolean to true since we've visited the next vertex
      for (Edge e: next.adjacentEdges) { // iterates through the edges
        if(!e.target.known) // if the target vertex hasn't already been visited
          if(next.distance + e.distance < e.target.distance){
            e.target.prev = next;
            e.target.distance = e.distance + next.distance;
            cities.add(e.target);
          } // end if loop
      } // end for loop
    } // end while loop
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    doDijkstra(s);
    List<Edge> retval = new LinkedList<Edge>(); // instantiate an instance of the return type
    Vertex pointer = getVertex(t); // establish the target vertex
    while (pointer.prev != null){
      retval.add(new Edge(pointer.prev, pointer, pointer.distance));
      pointer = pointer.prev; // work backwards through the prev pointers to find the path
    }
    return retval;
  }
